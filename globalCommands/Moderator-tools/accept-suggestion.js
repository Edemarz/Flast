...
module.exports = {
    ...
    .setDescription("Accept a member's suggestion!")
    ...
    .addStringOption((option) => option.setName("reason").setDescription("The reason to accept this suggestion!").setRequired(true)),
    ...
    async execute(client, interaction, Discord) {
        ...
        if (!findingMessageDB) return interaction.followUp({ embeds: [
            client.createEmbed({
                text: `${user}, You cannot accept the suggestion because someone has denied or accepted his/her suggestion!`,
                color: "RED",
                footerOne: guild.name,
                footerTwo: guild.iconURL({ dynamic: true }),
                thumbnail: guild.iconURL({ dynamic: true })
        ...
        const replacedData = `${embedData.description}`.replace("**N/A**", "**Accepted**");
        ...
        .setAuthor(`${guild.name} Server | Accepted Suggestion`, guild.iconURL({ dynamic: true }))
        .setThumbnail(guild.iconURL({ dynamic: true }))
        .setFooter(guild.name, guild.iconURL({ dynamic: true }))
        .setDescription(`${user}, You have accepted ${author ? author : "Internal Error"}'s Suggestion!`)
        .setColor("BLURPLE")
        .setTimestamp()
